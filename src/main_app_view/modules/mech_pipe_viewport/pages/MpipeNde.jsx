...
export default class MpipeNDE extends Component {

	constructor(props) {
		super(props)
		this.data = this.props.masterdata["nde_table"]
		
		this.fields = ["NDEiddate", 'NDRidno',]
		...
		this.headings = [
			headingObjectFactory("NDE/T Request Date", 0),
			headingObjectFactory("NDE/T Request No.", 1), 
		]

		this.tableId = "mpipe-nde-table"
	}
	...
	filterGenerator() {
		...
		filteredData.forEach(element => element['type'] = 'mpipe_ndes')
			...
			switch(element['type']) {
				case "mpipe_dwg":
					for( let connectedElement of connected_filter(this.props.masterdata["welds_table"], element, "DWGidno", "id")) {
						addData = connected_filter(data, connectedElement, "NDEWid", "id")
						filteredData = [...filteredData, ...addData]
					}
					break;
				case "mpipe_welds":
					addData = connected_filter(data, element, "NDEWid", "id")
					filteredData = [...filteredData, ...addData]
					break;
				case "mpipe_linelist":
					for(let connectedElement of connected_filter(this.props.masterdata["dwg_table"], element, "LNEidno", "LNEno")) {
						for( let nextConnectedElement of connected_filter(this.props.masterdata["welds_table"], connectedElement, "DWGidno", "id")) {
							addData = connected_filter(data, nextConnectedElement, "NDEWid", "id")
							filteredData = [...filteredData, ...addData]
						}
					}
					break;
				...
				case "mpipe_vts":
					...
					break;
				case "mpipe_ht":
					addData = connected_filter(data, element, "NDEWid", "Widno")
					filteredData = [...filteredData, ...addData]
					break;
					...
					for(let connectedElement of connected_filter(this.props.masterdata['line_list_table'], element, "INRidno", "id")) {
						for(let nextConnectedElement of connected_filter(this.props.masterdata["dwg_table"], connectedElement, "LNEidno", "LNEno")) {
							for( let nextNextConnectedElement of connected_filter(this.props.masterdata["welds_table"], nextConnectedElement, "DWGidno", "id")) {
								addData = connected_filter(data, nextNextConnectedElement, "NDEWid", "id")
								filteredData = [...filteredData, ...addData]
							}
						}
					}
				...
				<Container fluid>
					<Row>
						<Container className="floating-container" fluid>
							<Col>
								<SearchableTable type={"mpipe_ndes"} id={this.tableId} data={this.data} dataFilter={this.dataFilter} fields={this.fields} headings={this.headings}/>
							</Col>
						</Container>
					</Row>
