...
export default class MpipeBolts extends Component {

	constructor(props) {
		super(props)
		this.data = this.props.masterdata["bolts_table"]
		
		this.fields = [/*"BDiddate", 'BDflgtype',*/'BDconnno', 'BDrating', 'BDgasmat', 'BDsize', 'BDby',]
		...
		this.headings = [
			...
			// headingObjectFactory("Bolt Flange Type", 1), 
			headingObjectFactory("Bolt connection No.", 2), 
			headingObjectFactory("Rating", 3), 
			headingObjectFactory("Bolt Gasket Material", 4),
			headingObjectFactory("Size", 5), 
			headingObjectFactory("Completed By", 5), 
		]

		this.tableId = "mpipe-bolts-table"
	}
	...
	filterGenerator() {
		...
		filteredData.forEach(element => element['type'] = 'mpipe_bolts')
				...
				<Container fluid>
					<Row>
						<Container className="floating-container" fluid>
							<Col>
								<SearchableTable type={"mpipe_bolts"} id={this.tableId} data={this.data} dataFilter={this.dataFilter} fields={this.fields} headings={this.headings}/>
							</Col>
						</Container>
					</Row>
