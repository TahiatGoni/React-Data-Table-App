...
export default class MpipeVT extends Component {

	constructor(props) {
		super(props)
		this.data = this.props.masterdata["vts_table"]
		
		this.fields = ["RVTdate", 'RVTno']
		...
		this.headings = [
			headingObjectFactory("VT Request Date", 0),
			headingObjectFactory("VT Report No.", 2), 
		]

		this.tableId = "mpipe-vts-table"
	}
	...
	filterGenerator() {
		...
		filteredData.forEach(element => element['type'] = 'mpipe_vts')
	...
	dataFilter(data, ruleList, type="rule") {
		...
		for(let element of ruleList) {
			switch(element['type']) {
				case "mpipe_dwg":
					for(let connectedElement of [...connected_filter(this.props.masterdata['welds_table'], element, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['bonds_table'], element, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['fuses_table'], element, 'DWGidno', 'id')]) {
						addData = connected_filter(data, connectedElement, "id", "RVTidno")
						filteredData = [...filteredData, ...addData]
					} 
					break;
				case "mpipe_welds":
					addData = connected_filter(data, element, "id", "RVTidno")
					filteredData = [...filteredData, ...addData]
					break;
				case "mpipe_linelist":
					for(let connectedElement of connected_filter(this.props.masterdata['dwg_table'], element, 'LNEidno', 'LNEno')) {
						for(let nextConnectedElement of [...connected_filter(this.props.masterdata['welds_table'], connectedElement, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['bonds_table'], connectedElement, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['fuses_table'], connectedElement, 'DWGidno', 'id')]) {
							addData = connected_filter(data, nextConnectedElement, "id", "RVTidno")
							filteredData = [...filteredData, ...addData]
						} 
					} 
					break;
				...
				case "mpipe_fuses":
					...
					break;
				case "mpipe_bolts":
					for(let connectedElement of connected_filter(this.props.masterdata['dwg_table'], element, 'id', 'DWGidno')) {
						for(let nextConnectedElement of [...connected_filter(this.props.masterdata['welds_table'], connectedElement, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['bonds_table'], connectedElement, 'DWGidno', 'id'), ...connected_filter(this.props.masterdata['fuses_table'], connectedElement, 'DWGidno', 'id')]) {
							addData = connected_filter(data, nextConnectedElement, "id", "RVTidno")
							filteredData = [...filteredData, ...addData]
						} 
					...
					for(let connectedElement of connected_filter(this.props.masterdata["line_list_table"], element, "INRidno", "id"))
						for(let nextConnectedElement of connected_filter(this.props.masterdata["dwg_table"], connectedElement, "LNEidno", "LNEno")) {
							for(let nextNextConnectedElement of connected_filter(this.props.masterdata["welds_table"], nextConnectedElement, "DWGidno", "id")) {
								addData = connected_filter(data, nextNextConnectedElement, "id", "RVTidno")
								filteredData = [...filteredData, ...addData]
							}
						}
					break;
				...
				<Container fluid>
					<Row>
						<Container className="floating-container" fluid>
							<Col>
								<SearchableTable type={"mpipe_vts"} id={this.tableId} data={this.data} dataFilter={this.dataFilter} fields={this.fields} headings={this.headings}/>
							</Col>
						</Container>
					</Row>
