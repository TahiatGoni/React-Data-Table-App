...
export default class MpipeInsulation extends Component {

	constructor(props) {
		super(props)

		this.data = this.props.masterdata["insulation_table"]
		this.fields = ["INRiddatestamp", "INRno"]
		...
		this.headings = [
			...
			headingObjectFactory("INS Request Date", 1), 
			headingObjectFactory("INS Report No.", 2),
		]

		this.tableId = "mpipe-insulation-table"
	}
	...
	filterGenerator() {
		...
		filteredData.forEach(element => element['type'] = 'mpipe_insulation')
	...
	dataFilter(data, ruleList, type="rule") {
		...
		for(let element of ruleList) {
			switch(element['type']) {
				case "mpipe_dwg":
					for(let connectedElement of connected_filter(this.props.masterdata["line_list_table"], element, 'LNEno', 'LNEidno')) {
						addData = connected_filter(data, connectedElement, "id", "INRidno")
						filteredData = [...filteredData, ...addData]
					}
					break;
				case "mpipe_welds":
					for(let connectedElement of connected_filter(dwgList, element, 'id', 'DWGidno')) {
						for(let nextConnectedElement of connected_filter(this.props.masterdata["line_list_table"], connectedElement, 'LNEno', 'LNEidno')) {
							addData = connected_filter(data, nextConnectedElement, "id", "INRidno")
							filteredData = [...filteredData, ...addData]
						}
					}
					break;
				case "mpipe_linelist":
					addData = connected_filter(data, element, "id", "INRidno")
					filteredData = [...filteredData, ...addData]
					break;
				case "mpipe_bonds":
					for(let connectedElement of connected_filter(dwgList, element, 'id', 'DWGidno')) {
				...
				case "mpipe_ndes":
					for(let connectedElement of connected_filter(this.props.masterdata['welds_table'], element, 'id', 'NDEWid')) {
						for(let nextConnectedElement of connected_filter(dwgList, connectedElement, 'id', 'DWGidno')) {
							for(let nextNextConnectedElement of connected_filter(this.props.masterdata["line_list_table"], nextConnectedElement, 'LNEno', 'LNEidno')) {
								addData = connected_filter(data, nextNextConnectedElement, "id", "INRidno")
								filteredData = [...filteredData, ...addData]
							}
						}
					} 
				...
				<Container fluid>
					<Row>
						<Container className="floating-container" fluid>
							<Col>		
								<SearchableTable type={"mpipe_insulation"} id={this.tableId} data={this.data} dataFilter={this.dataFilter} fields={this.fields} headings={this.headings}/>
							</Col>
						</Container>
					</Row>
				</Container>
